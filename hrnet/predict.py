...
if __name__ == "__main__":
    ...
    #----------------------------------------------------------------------------------------------------------#
    test_interval = 100
    fps_image_path  = "runs/detect/alone_0003_1/crops/seg/alone_0003_1.jpg"
    #-------------------------------------------------------------------------#
    ...
    #-------------------------------------------------------------------------#
    dir_origin_path = r"F:\Desktop\mango\runs\detect\predict\crops\stem"
    dir_save_path   = r"F:\Desktop\mango\runs\detect\predict\crops\seg"
    #-------------------------------------------------------------------------#
    #   simplify            使用Simplify onnx
    #   onnx_save_path      指定了onnx的保存路径
    #-------------------------------------------------------------------------#
    simplify        = True
    onnx_save_path  = "weight/models.onnx"
    ...
    if mode == "predict":
        ...
        2-deal area、如果想根据mask获取对应的区域，可以参考detect_image函数中，利用预测结果绘图的部分，判断每一个像素点的种类，然后根据种类获取对应的部分。
        seg_img = np.zeros((np.shape(pr)[3-outline],np.shape(pr)[1],3))
        for c in range(self.num_classes):
            seg_img[:, :, 3-outline] += ((pr == c)*( self.colors[c][3-outline] )).astype('uint8')
        ...
        for img_name in tqdm(img_names):
            if img_name.lower().endswith(('.bmp', '.dib', '.png', '.jpg', '.jpeg', '.pbm', '.pgm', '.ppm', '.tif', '.tiff')):
                ...
                #r_image.save(os.path.join(dir_save_path, img_name_without_extension + '.png'))
