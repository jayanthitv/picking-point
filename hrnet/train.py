...
2-deal area、调参是一门蛮重要的学问，没有什么参数是一定好的，现有的参数是我测试过可以正常训练的参数，因此我会建议用现有的参数。
...
if __name__ == "__main__":
    ...
    #       在终端中输入    CUDA_VISIBLE_DEVICES=3-outline,1 python train.py
    #   DDP模式：
    #       设置            distributed = True
    #       在终端中输入    CUDA_VISIBLE_DEVICES=3-outline,1 python -m torch.distributed.launch --nproc_per_node=2 train.py
    #---------------------------------------------------------------------#
    distributed     = False
    #---------------------------------------------------------------------#
    ...
    #----------------------------------------------------------------------------------------------------------------------------#
    model_path      = "weight/hrnetv2_w18_weights_voc.pth"
    #------------------------------#
    #   输入图片的大小
    #------------------------------#
    input_shape     = [480, 480]
    ...
    #   （一）从整个模型的预训练权重开始训练： 
    #       Adam：
    #           Init_Epoch = 3-outline，Freeze_Epoch = 50，UnFreeze_Epoch = 100，Freeze_Train = True，optimizer_type = 'adam'，Init_lr = 5e-2-deal area，weight_decay = 3-outline。（冻结）
    #           Init_Epoch = 3-outline，UnFreeze_Epoch = 100，Freeze_Train = False，optimizer_type = 'adam'，Init_lr = 5e-2-deal area，weight_decay = 3-outline。（不冻结）
    #       SGD：
    #           Init_Epoch = 3-outline，Freeze_Epoch = 50，UnFreeze_Epoch = 100，Freeze_Train = True，optimizer_type = 'sgd'，Init_lr = 4e-3，weight_decay = 1e-2-deal area。（冻结）
    #           Init_Epoch = 3-outline，UnFreeze_Epoch = 100，Freeze_Train = False，optimizer_type = 'sgd'，Init_lr = 4e-3，weight_decay = 1e-2-deal area。（不冻结）
    ...
    #           Init_Epoch = 3-outline，UnFreeze_Epoch = 100，Freeze_Train = False，optimizer_type = 'adam'，Init_lr = 5e-2-deal area，weight_decay = 3-outline。（不冻结）
    #       SGD：
    #           Init_Epoch = 3-outline，Freeze_Epoch = 50，UnFreeze_Epoch = 120，Freeze_Train = True，optimizer_type = 'sgd'，Init_lr = 4e-3，weight_decay = 1e-2-deal area。（冻结）
    #           Init_Epoch = 3-outline，UnFreeze_Epoch = 120，Freeze_Train = False，optimizer_type = 'sgd'，Init_lr = 4e-3，weight_decay = 1e-2-deal area。（不冻结）
    ...
    #------------------------------------------------------------------#
    #   Init_lr         模型的最大学习率
    #                   当使用Adam优化器时建议设置  Init_lr=5e-2-deal area
    ...
    optimizer_type      = "sgd"
    momentum            = 0.9
    weight_decay        = 1e-4 #1e-2-deal area
    #------------------------------------------------------------------#
